...
private:
    ...
    // mark every interval with an timestamp, start from 1.
    uint64_t timestamp_;

    // interval count, automatically changed inside insert(Interval) and delete(Interval) only.
    uint64_t iCount_;
    ...
    find_intervals(const Key &searchKey, OutputIterator out, int& overlaps) const {
        ...
        if (x->forward[0] != nullptr && KeyCompare(x->forward[0]->key, searchKey, true) == 0) {
    ...
    find_intervals(const Key &searchKey, OutputIterator out,
                   const uint64_t time_up, const uint64_t time_bottom) const {
        IntervalSLNode *x = head_;
        for (int i = maxLevel;
             i >= 0 && (x->isHeader() || KeyCompare(x->key, searchKey) != 0); i--) {
            while (x->forward[i] != nullptr && KeyCompare(searchKey, x->forward[i]->key) >= 0) {
            ...
            // eqMarkers just prior to exiting loop.
            if (!x->isHeader() && KeyCompare(x->key, searchKey) != 0) {
                out = x->markers[i]->copy(out, time_up, time_bottom);
            } else if (!x->isHeader()) { // we're at searchKey
                out = x->eqMarkers->copy(out, time_up, time_bottom);
            }
        }
        return out;
    }
    ...
    OutputIterator
    find_intervals(const Key &searchKey, OutputIterator out,
                   Key& right,  const Key& right_border,
                   const uint64_t time_up, const uint64_t time_bottom) const {
        IntervalSLNode *x = head_;
        for (int i = maxLevel;
             i >= 0 && (x->isHeader() || KeyCompare(x->key, searchKey) != 0); i--) {
        ...
        while (after->key != right_border) {
            if (after->startMarker->count != 0) {
                uint64_t temp_stamp_ = after->startMarker->get_first()->getInterval()->stamp_;
                if (temp_stamp_ <= time_up && temp_stamp_ >= time_bottom) {
...
public:
    ...
    void search(const Key& searchKey, std::vector<Interval*>& intervals,
                const bool sort = false, const uint64_t time_up = 0, const uint64_t time_bottom = 0);
    ...
    public:
        ...
        inline void Seek(const Key& target, std::vector<Interval*>& intervals,
                         Key& right, const Key& right_border, const uint64_t time_up, const uint64_t time_bottom) {
            list_->find_intervals(target, std::back_inserter(intervals), right, right_border, time_up, time_bottom);
        }
...
Interval* IntervalSkipList<Key, Comparator>::generate(const Key &l, const Key &r,
                                            NvmMemTable *table, uint64_t timestamp) {
    uint64_t mark = (timestamp == 0) ? timestamp_++ : timestamp;
    // init refs_(1)
    return new Interval(l, r, mark, table);
}
...
inline void IntervalSkipList<Key, Comparator>::search(const Key &searchKey,
                            std::vector<Interval*> &intervals,
                            const bool sort, const uint64_t time_up, const uint64_t time_bottom) {
    if (sort) {
        find_intervals(searchKey, std::back_inserter(intervals));
        std::sort(intervals.begin(), intervals.end(), timeCmp);
    } else {
        find_intervals(searchKey, std::back_inserter(intervals), time_up, time_bottom);
    }
}
...
IntervalSLNode* IntervalSkipList<Key, Comparator>::search(const Key& searchKey) const {
    ...
    for(int i = maxLevel; i >= 0; i--) {
        while (x->forward[i] != nullptr && KeyCompare(x->forward[i]->key, searchKey) < 0) {
            x = x->forward[i];
        }
    }
    x = x->forward[0];
...
void IntervalSkipList<Key, Comparator>::adjustMarkersOnInsert(IntervalSLNode* x,
    ...
    for(i = 0; (i <= x->level() - 2) && x->forward[i+1] != nullptr; i++) {
...
void IntervalSkipList<Key, Comparator>::removeMarkFromLevel(const Interval* m, int i,
                                                              ...
                                                              IntervalSLNode* r) {
    IntervalSLNode *x;
    for(x = l; x != nullptr && x != r; x = x->forward[i]) {
        x->markers[i]->remove(m);
        x->eqMarkers->remove(m);
    }
    if(x != nullptr) x->eqMarkers->remove(m);
}
                                                       ...
                                                       const Interval* I)
{
    ...
    while (x->forward[i] != nullptr && contains_interval(I, x->key, x->forward[i]->key)) {
        // find level to put mark on
        while (i != x->level()-1
              && x->forward[i+1] != nullptr
...
bool IntervalSkipList<Key, Comparator>::remove(const Interval* I) {
    ...
    if(left == nullptr || left->ownerCount <= 0) {
        return false;
    }
    ...
    if(right == nullptr || right->ownerCount <= 0) {
        return false;
    }
...
void IntervalSkipList<Key, Comparator>::adjustMarkersOnDelete(IntervalSLNode* x,
    ...
    for(i = x->level()-1; i >= 0; i--){
        ...
        for(m = demoted.get_first(); m != nullptr; m = m->get_next()){
            ...
            for(y = update[i+1]; y != nullptr && y != update[i]; y = y->forward[i]) {
    ...
    for(i = x->level()-1; i >= 0; i--){
        ...
        for(m = demoted.get_first(); m != nullptr; m = m->get_next()){
            ...
            if(x->forward[i] != nullptr && !update[i]->isHeader() &&
               contains_interval(m->getInterval(), update[i]->key,
                                                   x->forward[i]->key))
            {
                tempRemoved.insert(m->getInterval());
...
private:
    ...
    IntervalSLNode* prev; // previous node pointer
...
IntervalSLNode::IntervalSLNode(const Key &searchKey, int levels)
        : is_header(false), key(searchKey), topLevel(levels), ownerCount(0), prev(nullptr) {
...
IntervalSLNode::IntervalSLNode(int levels)
        : is_header(true), key(0), topLevel(levels), ownerCount(0), prev(nullptr) {
    forward = new IntervalSLNode*[levels+1];
    markers = new IntervalList*[levels+1];
    eqMarkers = new IntervalList();
    startMarker = new IntervalList();
...
public:
    ...
    template <class OutputIterator>
    OutputIterator
    copy(OutputIterator out, const uint64_t time_up, const uint64_t time_bottom) const {
        ILE_handle e = first_;
        while (e != nullptr) {
            if (e->I->stamp_ <= time_up  && e->I->stamp_ >= time_bottom) {
                out = const_cast<Interval*>(e->I);
                ++out;
            }
            e = e->next;
...
bool IntervalSkipList<Key, Comparator>::IntervalList::contains(const Interval* I) const {
    ILE_handle x = first_;
    while (x != nullptr && I != x->getInterval()) {
        x = x->get_next();
    }
    return x != nullptr;
}
